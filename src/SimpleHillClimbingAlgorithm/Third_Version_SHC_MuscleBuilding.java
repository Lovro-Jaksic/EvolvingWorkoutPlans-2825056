...
public class Third_Version_SHC_MuscleBuilding {
    ...
    public static int[] changeOneExercise(int[] solution, int[][] userPlan, int[] userConfig, String[] exerciseList) {
        ...
        int exerciseIndex = generateRandomExerciseIndex(userConfig[6]);

        if (exerciseIndex > (userConfig[6] - 9)) {
            exerciseIndex -= 9;
        }
        ...
        if (randomNumber < (1.0 / 3.0)) {
            ...
            newSolution = changeSets(newSolution, userPlan, userConfig, exerciseIndex + 1);
            newSolution = changeReps(newSolution, userPlan, userConfig, exerciseIndex + 2);
            // Alter 6 genes
        } else if (randomNumber < (2.0 / 3.0)) {
            for (int i = 0; i < 6; i+=3) {
                newSolution = changeExercise(newSolution, exerciseList, exerciseIndex + i);
                newSolution = changeSets(newSolution, userPlan, userConfig, exerciseIndex + i + 1);
                newSolution = changeReps(newSolution, userPlan, userConfig, exerciseIndex + i + 2);
            }
            // Alter 9 genes
        } else {
            for (int i = 0; i < 9; i+=3) {
    ...
    public static int[] changeSets(int[] solution, int[][] userPlan, int[] userConfig, int setsIndex) {
        ...
        if (setsIndex < (userConfig[0] * userConfig[4] * 3)) {
            solution[setsIndex] = userPlan[0][0];
        } else if (setsIndex > ((userConfig[0] * userConfig[4] * 3) - 1) && setsIndex < (userConfig[0] + (userConfig[1]) * userConfig[4] * 3)) {
            solution[setsIndex] = userPlan[1][0];
        } else if (setsIndex > ((userConfig[0] + (userConfig[1]) * userConfig[4] * 3) - 1) && setsIndex < (userConfig[0] + (userConfig[1] + userConfig[2]) * userConfig[4] * 3)) {
            solution[setsIndex] = userPlan[2][0];
        } else {
            solution[setsIndex] = userPlan[3][0];
        }
    ...
    public static int[] changeReps(int[] solution, int[][] userPlan, int[] userConfig, int repsIndex) {
        ...
        if (repsIndex < (userConfig[0] * userConfig[4] * 3)) {
            ...
            solution[repsIndex] = randomReps.nextInt(maxReps - minReps + 1) + minReps;
        } else if (repsIndex > ((userConfig[0] * userConfig[4] * 3) - 1) && repsIndex < (userConfig[0] + (userConfig[1]) * userConfig[4] * 3) ) {
            int minReps = userPlan[1][1];
            int maxReps = userPlan[1][2];
            Random randomReps = new Random();
            solution[repsIndex] = randomReps.nextInt(maxReps - minReps + 1) + minReps;
        } else if (repsIndex > ((userConfig[0] + (userConfig[1]) * userConfig[4] * 3) - 1) && repsIndex < (userConfig[0] + (userConfig[1] + userConfig[2]) * userConfig[4] * 3)) {
